...
use Modules\BoardsRent\Http\Requests\CustomerSourceRequest;
use Modules\BoardsRent\Transformers\CustomerSourceResource;

class CustomerSourceController extends Controller
{

    public function __construct(private CustomerSource $model)
    {
        $this->model = $model;
    }
    ...
    public function create(CustomerSourceRequest $request)
    {
        ...
        return responseJson(200, 'created', new CustomerSourceResource($model));

    }

    public function update($id, CustomerSourceRequest $request)
    {
        $model = $this->model->find($id);
        if (!$model) {
            return responseJson(404, 'not found');
        ...
        return responseJson(200, 'updated', new CustomerSourceResource($model));
    }
