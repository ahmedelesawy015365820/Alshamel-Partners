...
class RpDebitNoteRepository implements RpDebitNoteInterface
{
    private $model;
    private $setting;
    private $model_break;
    ...
    public function all($request)
    {
        ...
        })->where('type_document','debitNote')->orderBy($request->order ? $request->order : 'updated_at', $request->sort ? $request->sort : 'DESC');
    ...
    {
        return $this->model->where('type_document','debitNote')->find($id);
    }

    public function create($request)
    {
       return DB::transaction(function () use ($request) {
                $model = $this->model->create(array_merge($request,['type_document'=>'debitNote']));
            return $model;
        });
    }
    ...
    public function update($request, $id)
    {
        return DB::transaction(function () use ($request, $id) {
            $model = $this->model->where('type_document','debitNote')->find($id);
            $model->update(array_merge($request,['type_document'=>'debitNote']));
            return $model;
        });
    }
    ...
    public function delete($id)
    {
        $model = $this->find($id);
        if ($model->breakDowns){
            $this->model_break->where([['break_id',$model->id],['break_type','debitNote']])->delete();
        }
        $model->delete();
    }
