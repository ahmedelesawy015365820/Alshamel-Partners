<template>
    ...
    <b-modal
        :id="id"
        :title="type != 'edit'?getCompanyKey('document_linked_categories_create_form'):getCompanyKey('document_linked_categories_edit_form')"
        title-class="font-18"
        body-class="p-4 "
        :hide-footer="true"
        @show="resetModal"
        ...
        <form>
            ...
            <div class="row">
                <div class="col-md-12" v-if="isVisible('name')">
                    <div class="form-group">
                        <label for="field-1" class="control-label">
                            {{ getCompanyKey("document_linked_categories_name_ar") }}
                ...
                <div class="col-md-12" v-if="isVisible('name_e')">
                    <div class="form-group">
                        <label for="field-2" class="control-label">
                            {{ getCompanyKey("document_linked_categories_name_en") }}
                ...
                <div class="col-md-12" v-if="isVisible('title')">
                    <div class="form-group">
                        <label for="field-1" class="control-label">
                            {{ getCompanyKey("document_linked_categories_title_ar") }}
                            <span v-if="isRequired('title')" class="text-danger">*</span>
                        </label>
                        <div dir="rtl">
                            <input @keyup="arabicTitleValue(create.title)" type="text" class="form-control" data-create="1" v-model="$v.create.title.$model" :class="{
                        ...
                        <template v-if="errors.title">
                            <ErrorMessage v-for="(errorMessage, index) in errors.title" :key="index">{{ errorMessage }}
                            </ErrorMessage>
                        </template>
                    </div>
                </div>
                <div class="col-md-12" v-if="isVisible('title_e')">
                    <div class="form-group">
                        <label for="field-2" class="control-label">
                            {{ getCompanyKey("document_linked_categories_title_en") }}
                            <span v-if="isRequired('title_e')" class="text-danger">*</span>
                        </label>
                        <div dir="ltr">
                            <input @keyup="englishTitleValue(create.title_e)" type="text" class="form-control englishInput"
                                   data-create="2"
                        ...
                        <template v-if="errors.title_e">
                            <ErrorMessage v-for="(errorMessage, index) in errors.title_e" :key="index">{{ errorMessage }}
                            </ErrorMessage>
                        </template>
                    </div>
                </div>
                <div class="col-md-12" v-if="isVisible('db_table')">
                    <div class="form-group">
                        <label for="field-2" class="control-label">
                            {{ getCompanyKey("document_linked_categories_db_table") }}
                            <span v-if="isRequired('db_table')" class="text-danger">*</span>
                        </label>
                        <div dir="ltr">
                            <input type="text" class="form-control englishInput"
                                   data-create="2"
                                   v-model="$v.create.db_table.$model" :class="{
                                  'is-invalid': $v.create.db_table.$error || errors.db_table,
                                  'is-valid': !$v.create.db_table.$invalid && !errors.db_table,
                                }"
                            />
                        </div>
                        <template v-if="errors.db_table">
                            <ErrorMessage v-for="(errorMessage, index) in errors.db_table" :key="index">{{ errorMessage }}
                            </ErrorMessage>
                        </template>
                    </div>
                </div>
...
export default {
    ...
    props: {
        ...
        type: {default: 'create'}, idObjEdit: {default: null},isPermission: {},url: {default: '/document-module-type'}
    },
    data() {
        return {
            fields: [],
    ...
    methods: {
        getCustomTableFields() {
            this.isCustom = true;
             adminApi
                .get(`/customTable/table-columns/general_document_module_types`)
                .then((res) => {
            ...
            setTimeout( () => {
                if(this.type != 'edit'){
                    ...
                    if(this.idObjEdit.dataObj){
                        ...
                        this.create.db_table = unitStatus.db_table;
                    }
                }
            },50);
        },
        ...
        arabicTitleValue(txt){
            this.create.title = arabicValue(txt);
        },
        englishTitleValue(txt){
