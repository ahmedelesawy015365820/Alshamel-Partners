...
import GuestAttendant from "./guest";
...
export default {
    name: "BookingAttendant",
  props: {
        id: {default: "attendant-create",}, companyKeys: {default: [],}, defaultsKeys: {default: [],},
        isPage: {default: true},isVisiblePage: {default: null},isRequiredPage: {default: null},
        type: {default: 'create'}, idObjEdit: {default: null},isPermission: {},url: {default: '/attendant'}
  },
  ...
  validations: {
    create: {
        name: {
            ...
            }),minLength: minLength(2),maxLength: maxLength(200),
        },
        name_e: {
            required: requiredIf(function (model) {
  ...
  methods: {
    ...
    resetModal() {
      ...
      setTimeout(async () => {
            if(this.type != 'edit'){
                ...
                if (this.isVisible("customer_id"))  this.getGuest();
            }else {
    ...
    AddSubmit() {
      ...
      getGuest() {
       adminApi
        .get(`/general-customer?type=1`)
        .then((res) => {
          let l = res.data.data;
          // if (this.isPermission("create guest")) {
    ...
    showGuestModal() {
      if (this.create.customer_id == 0) {
        this.$bvModal.show("attendant-guest-create");
...
<!--        :id="'attendant-guest-create'" @created="getGuest"-->
<!--    />-->
    <!--  create   -->
    <b-modal
      :id="id"
      :title="type != 'edit'?getCompanyKey('attendant_create_form'):getCompanyKey('attendant_edit_form')"
      title-class="font-18"
      size="lg"
      body-class="p-4 "
      :hide-footer="true"
              ...
              <div class="col-md-6" v-if="isVisible('customer_id')">
                  <div class="form-group">
                      ...
                      <template v-if="errors.employee_id">
                          <ErrorMessage v-for="(errorMessage, index) in errors.employee_id" :key="index">{{ errorMessage }}</ErrorMessage>
                      </template>
                  </div>
              </div>
              ...
              <div class="col-md-6" v-if="isVisible('id_number')">
                  <div class="form-group">
                      ...
                      <template v-if="errors.id_number">
                          <ErrorMessage v-for="(errorMessage, index) in errors.id_number" :key="index">{{ errorMessage }}</ErrorMessage>
                      </template>
                  </div>
              </div>
              <div class="col-md-6" v-if="isVisible('notes')">
                  <div class="form-group">
                      ...
                      <template v-if="errors.notes">
                          <ErrorMessage v-for="(errorMessage, index) in errors.notes" :key="index">{{ errorMessage }}</ErrorMessage>
                      </template>
                  </div>
              </div>
          </div>
