        ...
        <div class="row mx-0 modal-content-row" v-if="order">
            ...
            <div class="col-12 col-md-6 cart-border-right text-center">
                <div class="horizontal-scroll" id="printCustom">
                    <h5 class="text-center mb-4">{{ $t('general.' + order.orderType) + ' ' + $t('general.details') }}</h5>
                    ...
                    <div>
                        <div class="text-center header-line-height">
                            ...
                            <small class='text-center'>
                                {{ $t('general.' + order.orderType) + ' ' + $t('general.receipt') }}
                            </small>
                            <br>
...
<!--                            </small>-->
                        ...
                        <div class="invoice-table">
                            <table class="table product-card-font" style="font-weight:500">
                                ...
                                <tbody>
                                <tr v-for="cartItem in order.details">
                                    <template v-if="cartItem.id && cartItem.product_id">
                                    <td class="cart-summary-table text-left">
                                        ...
                                        <br>
                                        <span v-if="checkVariant(cartItem.product_id,cartItem.id)">( {{ checkVariant(cartItem.product_id,cartItem.id) }} )</span>
                                    </td>
            ...
            <div class="col-12 col-md-6">
                <div v-if="!status.isPaymentDone">
                    <div class="row mx-0 mb-4" id="js-payment-title">
                        ...
                        <div class="col-6 text-right payment-amount"><h5>
                            {{ totalAmount }}
                            {{ order.currency ? $i18n.locale == 'ar'? order.currency.symbol:order.currency.symbol_e : 'KU' }}
                        </h5></div>
                    </div>
                    <div>
                        <div class="payment-description" id="js-payment-description">
                            ...
                            <div v-if="order.orderType === 'sales'" class="form-group row ml-0">
    ...
    methods: {
        ...
        handelSellMethodData(){
            if (this.sellMethods.length > 0)
            {
                ...
                this.paymentValue = this.totalAmount - this.paidAmount;
            }
        ...
        getPaymentAmount() {
            ...
            this.due = this.totalAmount - (parseFloat(this.paidAmount) + parseFloat(this.paymentValue));
        ...
        resetModal() {
            setTimeout(() => {
                this.totalAmount = this.order.tax == "excluded" ? this.total : parseFloat(this.total) + parseFloat(this.taxProducts);
                this.handelSellMethodData();
                }, 200);
        },
        storeInvoice() {
            ...
            if(this.order.type != 'purchase'){
                dataCreate = {
                    ...
                    sub_discount:this.order.subDiscount,
                    due_amount:this.due,
                    ...
                    payments:payments,
                    tax_type: this.order.tax
                };
                if(!this.order.order_id){
                    this.isLoader = true;
                    adminApi
                        .post(`/point-of-sale/oreders`,dataCreate )
                        .then((res) => {
                            if(res.data){
                                this.serial = res.data.prefix;
                            }
                    ...
                    this.isLoader = true;
                    adminApi
                        .put(`/point-of-sale/oreders/update-hold/${this.order.order_id}`,dataCreate )
                        .then((res) => {
                            this.is_disabled = true;
                            if(res.data){
                                this.serial = res.data.prefix;
                ...
                this.isLoader = true;
                adminApi
                    .post(`/point-of-sale/oreders/create-purchases-products`,dataCreate )
                    .then((res) => {
                        this.is_disabled = true;
                        if(res.data){
                            this.serial = res.data.prefix;
