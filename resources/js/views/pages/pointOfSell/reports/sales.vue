...
export default {
  ...
  methods: {
    ...
    async getTotal() {
      ...
      await adminApi
        .get(
          `/point-of-sale/sales-list/grandTotal?start_date=${
            this.from_date ?? ""
          }
    ...
    getData(page = 1) {
      ...
      adminApi
        .get(
          `/point-of-sale/sales-list?page=${page}
    ...
    getDataCurrentPage(page = 1) {
      if (
        ...
        adminApi
          .get(
            `/point-of-sale/sales-list?page=${this.current_page}
          &per_page=${this.per_page}
          &start_date=${this.from_date ?? ""}
          &end_date=${this.to_date ?? ""}
          &payment_method_id=${this.payment_type ?? ""}
  ...
  <Layout>
    ...
    >
      <div class="row mb-2 px-2">
        ...
        <!-- <div class="col-md-2">
          <label>{{ $t("general.salesType") }}</label>
          <multiselect
            v-model="sales_type_id"
            :options="salesTypes.map((type) => type.id)"
            :custom-label="
              (opt) =>
                $i18n.locale == 'ar'
                  ? salesTypes.find((x) => x.id == opt).name
                  : salesTypes.find((x) => x.id == opt).name_e
            "
          >
          </multiselect>
        </div> -->
