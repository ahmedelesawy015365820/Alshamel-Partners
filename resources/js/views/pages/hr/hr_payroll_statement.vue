...
export default {
  page: {
    ...
    meta: [{ name: "description", content: "HR payrolls statement" }],
  },
  mixins: [translation],
  components: {
    Layout,
  ...
  methods: {
    ...
    getData(page = 1) {
      ...
      adminApi
        .get(
          `/hr/payroll/getAll?page=${page}&per_page=${this.per_page}&search=${this.search}&${filter}`
        )
        .then((res) => {
          let l = res.data;
          this.payrolls = l.data;
          this.payrollsPagination = l.pagination;
    ...
    getDataCurrentPage(page = 1) {
      if (
        this.current_page <= this.payrollsPagination.last_page &&
        this.current_page != this.payrollsPagination.current_page &&
        this.current_page
      ) {
        this.isLoader = true;
        let filter = "";
        ...
        adminApi
          .get(
            `/hr/payroll/getAll?page=${this.current_page}&per_page=${this.per_page}&search=${this.search}&${filter}&company_id=${this.company_id}`
          )
          .then((res) => {
            let l = res.data;
            this.payrolls = l.data;
      ...
      if (Array.isArray(id)) {
        Swal.fire({
          ...
          if (result.value) {
            this.isLoader = true;
            adminApi
              .post(`/hr/payroll/getAll/bulk-delete`, { ids: id })
              .then((res) => {
                this.checkAll = [];
                this.getData();
                Swal.fire({
    ...
    AddSubmit() {
      ...
      if (this.$v.create.$invalid) {
        ...
        adminApi
          .post(`/hr/payroll/getAll`, {...this.create,company_id: this.$store.getters["auth/company_id"]})
          .then((res) => {
    ...
    editSubmit(id) {
      ...
      if (this.$v.edit.$invalid) {
        ...
        this.errors = {};
        adminApi
          .put(`/hr/payroll/getAll/${id}`, this.edit)
          .then((res) => {
     ...
     */
    resetModalEdit(id) {
      let payrolls = this.payrolls.find((e) => id == e.id);
      this.edit.employee =
        this.$i18n.locale == "ar"
          ? payrolls.employee.name
          : payrolls.employee.name_e;
      ...
      this.edit.benefits = payrolls.benefits
        .map((b) => (lang == "ar" ? b.name : b.name_e))
        .join(",");
      this.edit.deductions = payrolls.deductions
        .map((d) => (lang == "ar" ? d.name : d.name_e))
        .join(",");
          ...
          <div class="card-body">
            ...
            <div class="row justify-content-between align-items-center mb-2">
              <h4 class="header-title">
                {{ $t("general.payrollStatementTable") }}
              </h4>
                ...
                <div class="d-inline-block" style="width: 22.2%">
                  ...
                  >
                    ...
                    <b-form-checkbox
                      ...
                      class="mb-1"
                    >
                      {{ getCompanyKey("payroll_statement_basic_salary") }}
                    </b-form-checkbox>
                    <b-form-checkbox
                      v-model="filterSetting"
                      value="work_days"
                      class="mb-1"
                    >
                      {{ getCompanyKey("payroll_statement_work_days") }}
                    </b-form-checkbox>
                    <b-form-checkbox
                      v-model="filterSetting"
                      value="work_value"
                      class="mb-1"
                    >
                      {{ getCompanyKey("payroll_statement_work_value") }}
                    </b-form-checkbox>
                    <b-form-checkbox
                      v-model="filterSetting"
                      value="net"
                ...
                <div class="d-fex">
                  ...
                  <div class="d-inline-block">
                    ...
                    >
                      ...
                      <b-form-checkbox v-model="setting.date" class="mb-1">
                        {{ getCompanyKey("payroll_statement_date") }}
                      </b-form-checkbox>
                      <b-form-checkbox
                        v-model="setting.basic_salary"
                        class="mb-1"
                      >
                        {{ getCompanyKey("payroll_statement_basic_salary") }}
                      </b-form-checkbox>

                      <b-form-checkbox v-model="setting.work_days" class="mb-1">
                        {{ getCompanyKey("payroll_statement_work_days") }}
                      </b-form-checkbox>

                      <b-form-checkbox
                        v-model="setting.work_value"
                        class="mb-1"
                      >
                        {{ getCompanyKey("payroll_statement_work_value") }}
                      </b-form-checkbox>

                      <b-form-checkbox v-model="setting.benefits" class="mb-1">
                        {{ getCompanyKey("payroll_statement_benefits") }}
                      </b-form-checkbox>

                      <b-form-checkbox
                        v-model="setting.deductions"
                        class="mb-1"
                      >
                  ...
                  >
                    <div class="d-inline-block" style="font-size: 13px">
                      {{ payrollsPagination.from }}-{{ payrollsPagination.to }}
                      /
                      {{ payrollsPagination.total }}
                    </div>
                    <div class="d-inline-block">
                      <a
                        href="javascript:void(0)"
                        :style="{
                          'pointer-events':
                            payrollsPagination.current_page == 1 ? 'none' : '',
                        }"
                        @click.prevent="
                          getData(payrollsPagination.current_page - 1)
                        "
                      >
                        <span>&lt;</span>
                      </a>
                      ...
                      <a
                        ...
                        :style="{
                          'pointer-events':
                            payrollsPagination.last_page ==
                            payrollsPagination.current_page
                              ? 'none'
                              : '',
                        }"
                        @click.prevent="
            ...
            >
              ...
              >
                      ...
                      <div class="d-flex justify-content-center">
                        ...
                        <div class="arrow-sort">
                          <i
                            ...
                            @click="payrolls.sort(sortString('name'))"
                          ></i>
                          <i
                ...
                <tbody v-if="payrolls.length > 0">
                  <tr
                    @click.capture="checkRow(data.id)"
                    @dblclick.prevent="$bvModal.show(`modal-edit-${data.id}`)"
                    v-for="(data, index) in payrolls"
                    :key="data.id"
                  ...
                  >
                    ...
                    <td v-if="setting.basic_salary">
                      <h5 class="m-0 font-weight-normal">
                        {{ data.basic_salary }}
                      </h5>
                    </td>

                    <td v-if="setting.work_days">
                      <h5 class="m-0 font-weight-normal">
                        {{ data.work_days }}
                      </h5>
                    </td>

                    <td v-if="setting.work_value">
                      <h5 class="m-0 font-weight-normal">
                        {{ data.work_value }}
                      </h5>
                    </td>

                    <td v-if="setting.benefits">
                      <h5 class="m-0 font-weight-normal">
                        {{
                          data.benefits
                            .map((b) =>
                              $i18n.locale == "ar" ? b.name : b.name_e
                            )
                            .join(",")
                        }}
                      </h5>
                    ...
                    <td v-if="setting.deductions">
                      <h5 class="m-0 font-weight-normal">
                        {{
                          data.deductions
                            .map((d) =>
                              $i18n.locale == "ar" ? d.name : d.name_e
                            )
                            .join(",")
                        }}
                      </h5>
                    ...
                    <td v-if="enabled3" class="do-not-print">
                      ...
                      <b-modal
                        ...
                        :title="getCompanyKey('payroll_statement_view_form')"
                        title-class="font-18"
