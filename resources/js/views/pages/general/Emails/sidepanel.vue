...
export default {
    ...
    computed: {
        unreadMessagesCount() {
            return this.emailData.filter(emil => emil.unread).length;
        },
    },
    data() {
        return {
...
<template>
    <div class="inbox-leftbar">
        ...
        <div class="mail-list mt-4">
            <router-link tag="a" to="/dashboard/Emails" class="text-danger">
                <i class="ri-inbox-fill align-bottom mr-2"></i>Inbox
                <span class="badge badge-soft-danger float-right ml-2">{{ unreadMessagesCount }}</span>
            </router-link>

            <router-link tag="a" to="/dashboard/StarredEmails">
