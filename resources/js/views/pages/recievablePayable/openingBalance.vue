...
export default {
    ...
    methods: {
        ...
        async resetModal() {
            ...
            {
                await this.getModuleType(this.document.document_module_type.db_table);
                    ...
                    this.is_disabled = false;
                     l.data.forEach((el) => {
                         this.edit.push({
                             ...
                             module_type_id: el.module_type_id,
                            ...
                            <form>
                                ...
                                <template v-for="(item, index) in create">
                                    ...
                                    <div class="row">
                                        <div class="col-md-1 p-0">
                                            <div class="form-group">
                                                <label v-if="index == 0" class="control-label">
                                                    {{getCompanyKey('installment_opening_client_type')}}
                                                    <span class="text-danger">*</span>
                                                </label>
                                                <multiselect
                                                    :show-labels="false"
                                                    :disabled="true"
                                                    v-model="$v.create.$each[index].client_type_id.$model"
                                                    ...
                                                    :class="{'is-invalid':$v.create.$each[index].client_type_id.$error || errors.client_type_id,}"
                                                >
                                                </multiselect>
                                        ...
                                        <div class="col-md-1 p-0" v-if="document && document.document_module_type_id">
                                            <div class="form-group position-relative">
                                                <label v-if="index == 0" class="control-label">
                                                ...
                                                <multiselect
                                                    :show-labels="false"
                                                    v-model="create[index].module_type_id"
                                                    :options="moduleTypes.map((type) => type.id)"
                                                    :custom-label="(opt) => $i18n.locale == 'ar' ?moduleTypes.find((x) => x.id == opt).name:moduleTypes.find((x) => x.id == opt).name_e"
                                                >
                                                </multiselect>
                                                    ...
                                                    <div class="row">
                                                        <div class="col-md-1 p-0">
                                                            <div class="form-group">
                                                                ...
                                                                <multiselect
                                                                    :show-labels="false"
                                                                    :disabled="true"
                                                                    v-model="$v.edit.$each[index].client_type_id.$model"
                                                                    :options="clientTypes.map((type) => type.id)"
                                                                    :custom-label=" (opt) => clientTypes.find((x) => x.id == opt) ? $i18n.locale == 'ar' ? clientTypes.find((x) => x.id == opt).name: clientTypes.find((x) => x.id == opt).name_e:''"
                                                                    :class="{'is-invalid':$v.edit.$each[index].client_type_id.$error || errors.client_type_id,}"
                                                                >
                                                                </multiselect>
                                                                <template v-if="errors.client_type_id">
                                                                    <ErrorMessage v-for="(errorMessage, index) in errors.client_type_id"
                                                        ...
                                                        <div class="col-md-1 p-0" v-if="document && document.document_module_type_id">
                                                            <div class="form-group position-relative">
                                                                ...
                                                                <multiselect
                                                                    :show-labels="false"
                                                                    v-model="edit[index].module_type_id"
                                                                    :options="moduleTypes.map((type) => type.id)"
                                                                    :custom-label="(opt) => $i18n.locale == 'ar' ?moduleTypes.find((x) => x.id == opt).name:moduleTypes.find((x) => x.id == opt).name_e"
                                                                >
                                                                </multiselect>
